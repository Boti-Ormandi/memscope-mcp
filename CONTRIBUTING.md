...
- [`src/tools/`](src/tools/) -- tool implementations (`memory.py`, `scanning.py`, `pointers.py`, `types.py`, `execute.py`, `hooking.py`, `lua_scripts.py`)
- [`src/tools/lua/`](src/tools/lua/) -- Lua engine (`engine.py`) plus themed function modules: `memory_read`, `memory_write`, `process_info`, `scanning_helpers`, `struct_helpers`, `modules`, `code_execution`, `comparisons`, `utilities`, `hooking`, `network`
- [`src/extensions/`](src/extensions/) -- `LuaExtension` ABC + bootstrap + the seven core extensions under `core/`
- [`src/utils/`](src/utils/) -- address parsing, heuristics, x64 shellcode (`shellcode.py`), instruction decoder + relocator (`disasm.py`), PE export resolver (`pe.py`), PEB reader (`peb.py`)
...
5. Document the function in [`docs/lua-reference.md`](docs/lua-reference.md) under the matching category and in [`src/instructions/base.py`](src/instructions/base.py) if a shared-guidance bullet is appropriate.
...
1. Create `src/extensions/core/<your_ext>.py`. Subclass `LuaExtension` from `src/extensions/base.py`. Implement `name`, `description`, `instructions`, and `register(ctx)`. Override `on_process_attached` / `on_process_detaching` if the extension holds process-bound state (allocations, hooks).
2. Register the class in `src/extensions/core/__init__.py` -- import it and add it to `CORE_EXTENSIONS` in the right position (`General` first, the rest in the order the AI is likely to encounter them).
...
`tests/conftest.py` imports `src.server` once at collection time so the extension bootstrap runs before any test resolves a Lua function. If a new test needs the Lua surface initialized, it relies on this import side-effect -- nothing else is required.
